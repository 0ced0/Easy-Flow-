    ...
    try{
        ...
        return(
            <div className="popUpRoot">
                ...
                <div className="popUpContainerVM w-[96vw] max-h-[92vh] overflow-y-auto">
                    <div className="flex flex-wrap items-center gap-2 border-b border-[#D9D9D9] px-3 sm:px-4 pb-2 justify-between overflow-y-auto pt-1">
                        <h1>Violation Monitoring</h1>
                        <button className="ml-0 sm:ml-5 border-none outline-none focus:outline-none focus:ring-0 shadow-[0px_1px_4px_1px_rgba(0,0,0,0.25)] rounded-[8px] bg-white flex gap-2 justify-between items-center px-3 sm:px-4 py-1 hover:bg-black/10">
                            Date
                        ...
                        <button onClick={() => {setShowViolationPopUp(false)}} className="ml-auto sm:mr-4">
                    ...
                    <div className="h-auto lg:h-[70vh] p-2 sm:p-4 grid grid-cols-1 lg:grid-cols-2 gap-4">
                        <div className="px-0 sm:px-4 flex w-full lg:w-[55vw] ml-0 lg:ml-10 overflow-x-auto">
                            <div className="shadow-[0px_1px_4px_1px_rgba(0,0,0,0.25)] rounded-[15px] p-3 w-full h-full">   
                                {compiledIllegalParkingList.map(([vehicleId, violationInformation]) => {
                                    return(violationInformation.violationStatus > 1 && (
                                        <button onClick={() => {setViolationFocus([vehicleId, violationInformation])}} className="grid grid-cols-5 gap-2 p-2 items-center min-w-[40rem] w-[100%] m-auto hover:bg-black/10 rounded-[15px]"
                                            key={vehicleId}>
                                            ...
                                            <p className="vmVehicle mr-2 lg:mr-[100px]">{violationInformation.vehicle}</p>
                        ...
                        <div className="space-y-5">
                            <div className="shadow-[0px_1px_4px_1px_rgba(0,0,0,0.25)] rounded-[15px] w-full lg:max-w-[29vw] h-auto min-h-52 lg:h-[33vh] p-2 sm:p-4 lg:ml-auto lg:mr-4">
                                {violationFocus[1] && (
                                    <img
                                        src={`data:image/jpeg;base64,${violationFocus?.[1]?.frame ?? ""}`}
                                        className="w-full h-full object-contain shadow-[0px_1px_4px_1px_rgba(0,0,0,0.25)] rounded-[15px]"
                                    />
                                )}
                            </div>
                        </div>
